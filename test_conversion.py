...
import sys
import logging
from handball_converter import process_file, load_system_prompt
...
def test_conversion(file_path):
    ...
    try:
        ...
        logger.info("System prompt indlæst korrekt")
    except Exception as e:
        logger.error(f"Fejl ved indlæsning af system prompt: {str(e)}")
        return
    ...
    if db_path:
        ...
        # Foreslå at køre check_database.py for at validere resultatet
        logger.info("\nFor at validere resultatet, kør:")
        logger.info(f"python check_database.py {db_path}")
    else:
...
def main():
    """Hovedfunktion"""
    if len(sys.argv) < 2:
        print("Brug: python test_conversion.py <sti_til_tekstfil>")
        print("Eksempel: python test_conversion.py Kvindeliga-txt-tabel/2024-2025/match_748182_a.txt")
        return
